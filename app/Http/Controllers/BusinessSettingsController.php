...
class BusinessSettingsController extends Controller
{
    ...
    public function payment_method_update(Request $request)
    {
        ...
        $business_settings = BusinessSetting::where('type', $request->payment_method)->first();
        if($business_settings != null) {
            if ($request->has($request->payment_method)) {
                $business_settings->value = 1;
                $s = $business_settings->save();
            }
            else{
    ...
    {
        $business_settings = BusinessSetting::where('type', 'sms_setting')->first();
        return view('business_settings.sms_setting',compact('business_settings'));
    }
    ...
    public function sms_setting_update(Request $request)
    {
        ...
        $val = json_encode($val_array);
        if ($business_settings) {
            $business_settings->value = $val;
            $business_settings->save();           
        }else{
            $sms_setting = array('type'=>'sms_setting','value'=>$val);
            $business_settings = BusinessSetting::create($sms_setting);
        }
        return view('business_settings.sms_setting',compact('business_settings'));
    }
    
    
    public function aramex_setting(Request $request)
    {
        $business_settings = BusinessSetting::where('type', 'aramex_setting')->first();
        return view('business_settings.aramex-setting',compact('business_settings'));
    }

    public function aramex_setting_update(Request $request)
    {
        $business_settings = BusinessSetting::where('type', 'aramex_setting')->first();
        $val_array = $request->all();
        $val = json_encode($val_array);
        ...
        if ($business_settings) {
            ...
            $aramex_setting     = array('type'=>'aramex_setting','value'=>$val);
            $business_settings  = BusinessSetting::create($aramex_setting);
        }
        flash('Aramex setting updated successfully')->success();
        return view('business_settings.aramex-setting',compact('business_settings'));
    }
