(function ($) {
   ...
   return false;
    ...
    $offcanvasNav.on('click', 'li .menu-expand, li .cat,li .maicat,li .subcat', function(e) {
        var $this = $(this);
        if ( ($this.parent().attr('class').match(/\b(menu-item-has-children|has-children|has-sub-men|menu-item-has-children1)\b/)) && ($this.attr('href') === '#' || $this.hasClass('menu-expand')) ) {
            e.preventDefault();
        ...
        if( $this.is('a.maicat') || $this.is('a.cat') || $this.is('a.subcat') ||  $this.attr('class').match(/\b(menu-expand)\b/) ){
        	$this.parent().toggleClass('menu-open');
        }else if( $this.is('li') && $this.attr('class').match(/\b('menu-item-has-mainchildren')\b/) ){
        	$this.toggleClass('menu-open');
        }
    });
    $offcanvasNavSubMenu = $offcanvasNav.find('.sub-sub-menu');
    $offcanvasNavSubMenu.parent().prepend('<span class="menu-expand1"><i class="fa fa-angle-down"></i></span>');
    $offcanvasNavSubMenu.slideUp();
...
//    for mega menu
    ...
    $(".brand-category").off('click.bs.tab.data-api', '[data-hover="tab"]');
    $(".brand-category").on('mouseenter.bs.tab.data-api', '[data-toggle="tab"], [data-hover="tab"]', function () {
        $(this).tab('show');
    });
    ...
    $('.shop-category>li').hover(function(){
        ...
        var menuht = $(".shop-category>li.active").find(".shop-mega-menu.hover").height();
        var conht = $(this).parents(".shop-category-contain").height();

        if( menuht > conht){
            $(this).parents(".shop-category-contain").height(menuht);
        } else {
            $(this).parents(".shop-category-contain").height(conht);
        }
    });
    ...
    $(window).load(function(){
        var menuht = $(".shop-category>li.active").find(".shop-mega-menu.hover").height();
        var conht = $(".shop-category>li").parents(".shop-category-contain").height();
        if( menuht > conht){
            $(".shop-category>li").parents(".shop-category-contain").height(menuht);
        } else {
            $(".shop-category>li").parents(".shop-category-contain").height(conht);
        }
    });
    ...
    var incrementPlus = $(document).on('click','.cart-qty-plus',function() {
        var $n = $(this)
            .parent(".button-container")
            .parent(".quantity-container")
            .find(".qty");
        $n.val(Number($n.val())+1 );
        updateQuantity($n.attr('data-key'), $n.val());
    });

    var incrementMinus = $(document).on('click','.cart-qty-minus',function() {
        var $n = $(this)
            .parent(".button-container")
            .parent(".quantity-container")
            .find(".qty");
